...
class DIPWatermark(LogitsWatermark):
	...
	def __init__(
		self, key: str, gamma=0.5, alpha=0.45, ignore_history_generation=False,
	...
	def embed(self, prompt: str) -> str:
		...
		# encode prompt
		encoded_prompt = llm_service.tokenizer(
			prompt, return_tensors="pt", add_special_tokens=True
		).to(
			llm_service.device
		)
		# 生成水印文本
		encoded_watermarked_text = llm_service.model.generate(
			**encoded_prompt, **self.generation_config.to_dict()
		)
		# 解码
		watermarked_text = \
		llm_service.tokenizer.batch_decode(encoded_watermarked_text, skip_special_tokens=True)[0]
		# 清除记录
	...
	def detect(self, text: str, **kwargs) -> Dict[str, Any]:
		...
		encoded_text = \
		llm_service.tokenizer(text, return_tensors="pt", add_special_tokens=False)["input_ids"][
			0].to(llm_service.device)
	...
	def reweight_logits(
		...
		portion_in_right_2 = (torch.gather(s_cumsum, -1, boundary_2) - (
					1 - self.alpha)) / p_boundary_2
	...
	def get_seed_for_cipher(self, input_ids: torch.LongTensor) -> Tuple[
		torch.FloatTensor, torch.FloatTensor]:
	...
	def score_sequence(self, input_ids: torch.LongTensor) -> tuple[float, list[int]]:
		...
		if not self.ignore_history_detection:
			...
			z_score = (green_tokens - (1 - self.gamma) * input_ids.size(-1)) / sqrt(
				input_ids.size(-1)
			)
